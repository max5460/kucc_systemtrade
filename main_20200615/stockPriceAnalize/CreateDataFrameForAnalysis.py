from CommonConstant import (
                            ...
                            PREVIOUS_DAYS_CLOSE_PRICE_SUFFIX,
...
def __GetDifferenceColumnBetweenTodayAndPreviousDay(sourceDataFrame):
    ...
    todayColumns = [columnName for columnName in sourceDataFrame.columns if PREVIOUS_COLUMN_PREFIX not in columnName and columnName != DROP_COLUMN_NAME_DATE]
...
def GetDataFrameForAnalysis(sourceDataFrame):
    ...
    dateSeries = technicalIndexDataFrame[DROP_COLUMN_NAME_DATE]
    ...
    technicalIndexDataFrame[DROP_COLUMN_NAME_DATE] = dateSeries
...
def GetSeveralDaysClosePriceDataFrame(sourceDataFrame):

    if sourceDataFrame is None or len(sourceDataFrame) == 0:
        logger.info(CreateDataFrameAnalysisMessage.sourceDataFrameError)
        return None

    if len(sourceDataFrame) < CLOSE_PRICE_COUNT_IN_A_ROW:
        return None

    dataFrameForAnalysis = sourceDataFrame.copy()

    dataFrameForAnalysis.sort_values(DROP_COLUMN_NAME_DATE, ascending=False, inplace=True)
    dataFrameForAnalysis.reset_index(drop=True, inplace=True)

    for idx in range(1, CLOSE_PRICE_COUNT_IN_A_ROW + 1):
        previousCloseSeries = dataFrameForAnalysis[TECHNICAL_INDEX_COLUMN_NAME_CLOSE].loc[idx:]
        previousCloseSeries.reset_index(drop=True, inplace=True)
        dataFrameForAnalysis[str(idx) + PREVIOUS_DAYS_CLOSE_PRICE_SUFFIX] = previousCloseSeries

    dataFrameForAnalysis.dropna(inplace=True)
    dataFrameForAnalysis.reset_index(drop=True, inplace=True)

    dataFrameForAnalysis.sort_values(DROP_COLUMN_NAME_DATE, ascending=True, inplace=True)
    dataFrameForAnalysis.reset_index(drop=True, inplace=True)

    return dataFrameForAnalysis



def GetSeveralDaysClosePriceDataFrame2(sourceDataFrame):
    ...
    dataFrameForAnalysis = sourceDataFrame[[DROP_COLUMN_NAME_DATE, 'ADX']].copy()

    dataFrameForAnalysis.sort_values(DROP_COLUMN_NAME_DATE, ascending=False, inplace=True)
    dataFrameForAnalysis.reset_index(drop=True, inplace=True)

    for idx in range(1, CLOSE_PRICE_COUNT_IN_A_ROW + 1):
        previousCloseSeries = dataFrameForAnalysis['ADX'].loc[idx:]
        previousCloseSeries.reset_index(drop=True, inplace=True)
        dataFrameForAnalysis[str(idx) + '日前ADX'] = previousCloseSeries
